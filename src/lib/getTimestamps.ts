...
export default function getTimestamps(day: string, time: string): WeekTime[] {
  ...
  const times = time.replace(/ |am|pm/gi, "").split("-");
  ...
  let start: any = null;
  let end: any = null;
  const t0 = Number.parseFloat(times[0]);
  const t1 = Number.parseFloat(times[1]);
